...
const AdminCartPage: React.FC = () => {
  ...
  const [searchedCartData, setSearchedCartData] = useState<any | null>(null);
  const [searchError, setSearchError] = useState<string | null>(null);
  const [searchLoading, setSearchLoading] = useState<boolean>(false);

  const carritoIds = [1, 2, 3, 4, 5, 6, 7, 17]; // IDs manuales

  const fetchCarritoById = async (id: number) => {
    try {
    ...
    try {
      ...
      const carrito = {
        ...
        productos: nuevoCarrito.carroProductos
          ? nuevoCarrito.carroProductos.map((p: any) => ({
              nombre: p.producto.nombre,
              cantidad: p.cantidadProducto,
              precio: p.producto.precio,
            }))
            ...
            <tbody>
              {carritos.map((carrito) => (
                <tr key={carrito.id}>
                  ...
                  <td>
                    <ul>
                      {carrito.productos.map((producto: any, index: number) => (
                        <li key={index}>
                          {producto.nombre} - {producto.cantidad} unidad(es) - ${producto.precio}
                        </li>
                      ))}
